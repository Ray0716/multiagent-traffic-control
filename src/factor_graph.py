...
class factor_graph():
    ...
    def __init__(self, parameters, input_shape, lr, discount, exploration_rate, mem_size, num_batches):
        self._parameters = parameters
        self.factored_graph = self._parameters['factored_agents']
        self.num_agents = len(self._parameters['lightPositions'].keys())
        for keys in self._parameters['testmodelnr'].keys():
            self.index = keys
        self.car_pr = self._parameters['car_pr']
        ...
        self.modelnr = self._parameters['testmodelnr']
        self.algorithm = self._parameters['coordination_algo']
        self.num_factors = len(self.factored_graph.keys())
        ...
        self.generate_agents(input_shape, lr, discount, exploration_rate, mem_size, num_batches)
        #self.load_models()
    ...
    def generate_agents(self, input_shape, lr, discount, exploration_rate, mem_size, num_batches):
        ...
        if self.algorithm == 'individual':
            ...
            agent = AgentDQN(input_shape=input_shape, num_actions=2, num_agents=2, lr=lr, gamma=discount, epsilon=1.0, epsilon_min=exploration_rate,
                             mem_size=mem_size, batch_size=num_batches)
            self.Q_function_dict['vertical'] = agent
    ...
    def get_q_value(self, local_state):
        ...
        if self.algorithm == 'individual':
            ...
            q_value = self.Q_function_dict['vertical'].predict(local_state)
