...
class experimentor():
    ...
    def __init__(self, total_simulation=8):
        ...
        with open("configs/four_config.yaml", 'r') as stream:
            try:
        ...
        OUTPUT_DIR = os.path.join(CURRENT_DIR, "../Output", parameters['scene'], parameters['coordination_algo'])
        OUTPUT_DIR = os.path.abspath(OUTPUT_DIR)
        os.makedirs(OUTPUT_DIR, exist_ok=True)
        RESULT_DIR = os.path.join(OUTPUT_DIR, "Results")
        os.makedirs(RESULT_DIR, exist_ok=True)
        MODEL_DIR = os.path.join(OUTPUT_DIR.replace(parameters['coordination_algo'], "Train").replace(parameters['scene'], parameters['factor_scene']), "Model")
        os.makedirs(MODEL_DIR, exist_ok=True)
        CHECKPOINT_DIR = os.path.join(OUTPUT_DIR.replace(parameters['coordination_algo'], "Train").replace(parameters['scene'], parameters['factor_scene']), "Checkpoint")
    ...
    def tester(self):
        ...
        eval_index = 99
        for self.model in range(10000, -1, -10000):
            eval_index += 1
